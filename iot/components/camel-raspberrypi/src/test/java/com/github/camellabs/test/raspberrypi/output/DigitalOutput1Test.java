        ...
        return new RouteBuilder() {
            public void configure() {
                from("timer://foo?repeatCount=1").id("rbpi-route").to("log:com.github.camellabs.component.raspberrypi?showAll=true&multiline=true")
                    .to("raspberrypi://pin/17?mode=DIGITAL_OUTPUT&state=LOW&action=TOGGLE").to("raspberrypi://pin/18?mode=DIGITAL_OUTPUT&state=HIGH&action=LOW")
                    .to("raspberrypi://pin/19?mode=DIGITAL_OUTPUT&state=LOW&action=HIGH").to("mock:result");
