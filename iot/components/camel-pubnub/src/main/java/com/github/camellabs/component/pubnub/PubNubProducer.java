...
public class PubNubProducer extends DefaultProducer {
	...
	public void process(final Exchange exchange) throws Exception {
		...
		case PUBLISH: {
			...
			LOG.trace("Sending message [{}] to channel [{}]", body, endpoint.getChannel());
			if (body.getClass().isAssignableFrom(JSONObject.class)) {
				endpoint.getPubnub().publish(endpoint.getChannel(), (JSONObject) body, callback);
			} else if (body.getClass().isAssignableFrom(JSONArray.class)) {
				endpoint.getPubnub().publish(endpoint.getChannel(), (JSONArray) body, callback);
			} else {
				endpoint.getPubnub().publish(endpoint.getChannel(), exchange.getIn().getMandatoryBody(String.class),
						callback);
			}
			break;
		}
		...
		case SET_STATE: {
			...
			endpoint.getPubnub().setState(endpoint.getChannel(), endpoint.getUuid(), state, callback);
			break;
		}
		case WHERE_NOW: {
			endpoint.getPubnub().whereNow(endpoint.getUuid(), callback);
			break;
		}
		default:
