...
export KURA_PLUGIN_FOLDER=${KURA_PLUGIN_FOLDER:-/opt/rhiot/plugins}

cd /opt/eclipse/kura/kura

#sed -i "" -e "s/.*org.osgi.service.http.port.*=.*\$/org.osgi.service.http.port=${KURA_HTTP_PORT}/" "config.ini"

if [ -d ${KURA_PLUGIN_FOLDER} ]; then
  for i in `ls ${KURA_PLUGIN_FOLDER} | egrep "*.jar"`; do
    cat config.ini | awk '/^osgi.bundles=/ { print $0 ", reference\\:file\\:'"${KURA_PLUGIN_FOLDER}/${i}"'@5\\:start" }   !/osgi.bundles=/ { print $0  }' > config.ini.tmp
