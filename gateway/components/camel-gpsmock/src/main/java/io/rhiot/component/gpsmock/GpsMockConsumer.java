...
public class GpsMockConsumer extends DefaultScheduledPollConsumer {
    ...
    public GpsMockConsumer(DefaultEndpoint defaultEndpoint, Processor processor) {
        super(defaultEndpoint, processor);
    }

    @Override
    protected int poll() throws Exception {
        GpsCoordinates coordinates = gpsCoordinates(RANDOM.nextDouble(), RANDOM.nextDouble());
        Exchange exchange = anExchange(getEndpoint().getCamelContext()).withBody(coordinates).build();
